...
{
    uint32_t id;
    char username[COLUMN_USERNAME_SIZE + 1]; // array of 32 chars (reserves 32 bytes of memory)
    char email[COLUMN_EMAIL_SIZE + 1];       // array of 255 chars (reserves 255 bytes of memory)
} Row;
...
const uint32_t PAGE_SIZE = 4096; // 4kb is same size in virtual memory systems
 ...
 * These nodes will store how many "cells" they have
 * Cells are (key, value) pairs
 */
 ...
 * The body of a leaf is an array of cells.
...
void *cursor_value(Cursor *cursor)
{
    ...
    uint32_t page_num = row_num / ROWS_PER_PAGE; // find page num

    void *page = get_page(cursor->table->pager, page_num);

    uint32_t row_offset = row_num % ROWS_PER_PAGE; // finds the row position within the page
    uint32_t byte_offset = row_offset * ROW_SIZE;  // find exact location of row in memory
